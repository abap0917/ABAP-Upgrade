...
*&---------------------------------------------------------------------*
*& Gateway OData operations for ZMCP_ADT_SRV:
...
START-OF-SELECTION.
  ...
  WRITE: / '=== OData Gateway flush + register + diag ==='.
  ULINE.
...
FORM flush_all_caches.
  ...
  " 模型缓存(/IWFND)—— 老版本 Gateway 的 CLEAN_UP 无参数
  TRY.
  ...
  " V2/V4 门面类仅存在于新版系统(如本系统不存在),已省略:
  "   /iwbep/cl_v2_cp_facade_factory=>create( )->create_config_facade( )->delete_all_model_data_cache( ).
  "   /iwbep/cl_v4_service_alias_fac=>create_for_runtime( )->clear_cache( ).
