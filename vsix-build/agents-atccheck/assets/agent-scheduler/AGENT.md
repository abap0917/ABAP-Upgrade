...
启动
 ...
 │
 ├─ ① 确认输入（向用户收集一次，透传给子 agent；可用 `scripts/scheduler-run.js [项目目录]` 一键初始化）
 │     - 项目名称 = projectKey（**未填时默认 = 当前根目录文件夹名**）
 │     - 项目路径（**默认 = 当前项目目录**）
 │     - 连接（.env 路径或参数；**执行前确保当前项目已创建 .env**，缺失时 scheduler-run.js 自动生成模板）
 ...
 │       或一键：node scripts/scheduler-run.js <项目目录> [--variant=] [--creator=] [--transport=]
 │         （自动：建 .env（缺失时）→ registry plan，名称默认根目录名、路径默认当前目录）
 │
 ├─ ② 注册表检查（含断点判断）
